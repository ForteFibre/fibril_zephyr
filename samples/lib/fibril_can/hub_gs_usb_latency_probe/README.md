# hub_gs_usb_latency_probe

`fibril_can_benchmark/docs/firmware-requirements.md` の contract を満たす MCU
firmware を、`fcan_hub` の **self node** として置き、外部 port を
...
物理層が「単一 CAN bus」ではなく「USB gs_usb + peer FDCAN1」になる。
...
hub は SPEC §2.5 の N-port 対称ブリッジで、単一論理バスの帯域が足りていて
ペア physical segment を接続したいだけの構成向け。帯域を分割したい場合は
router 版 (ADR-0013) を選ぶ。
...
FCAN_CODEGEN=/abs/path/fcan_codegen \
  west build -b fibril_robomaster_miniv1 \
    fibril_zephyr/samples/lib/fibril_can/hub_gs_usb_latency_probe
```
...
```sh
FCAN_CODEGEN=/abs/path/fcan_codegen \
  twister -T fibril_zephyr/samples/lib/fibril_can/hub_gs_usb_latency_probe
```
...
2. `dmesg | grep gs_usb` — `1209:ca01` の device が enumerate する。
   ...
   `hub_gs_usb_self` firmware と区別できる。
...
## latency components (§8 — probe が hub のどちら側にいるか)

このサンプルは **probe を hub の external port (self node) に置く**。
物理 CAN wire を経由しないため:

| 経路 | 単一 bus (`latency_probe_node`) | hub+gs_usb (このサンプル) |
| :--- | :---: | :---: |
| PC → MCU | CAN wire | USB FS (bulk) |
| MCU → PC | CAN wire | USB FS (bulk) |
| commit → emit dwell 最悪 | ≈ scheduler inspect | ≈ hub poll interval |
...
- **hub**: `probe_logic_tick` は app thread、`fcan_poll(self)` は hub
  driver thread。両者は `CONFIG_CAN_FCAN_HUB_POLL_INTERVAL_US` (default 1000 µs)
  ごとに独立に動く。したがって commit → emit の待ちは平均 0.5 ms、最悪 1 ms。

  この値は `prj.conf` の `CONFIG_CAN_FCAN_HUB_POLL_INTERVAL_US` で
...
`~1 ms (USB in) + ~1 ms (hub dwell) + ~1 ms (USB out) ≒ 3 ms` オーダー。
...
- Hot path (main の `probe_logic_tick` と hub thread の `fcan_poll(self)`)
...
- **probe は hub の external port (self node) 側**。ROS 側は
  `probe が hub のどちら側にいたか` を「external (self)」として実験ログに書く。
- fdcan1 peer port に別の slave (例: `example_node`) を繋ぐと、その topic は
  同じ `canX` に載って ROS 側に到達する。実験時 peer 側の負荷が
  self の echo emit に干渉しないかは `fcan_hub_get_diag` の
...
| arb bitrate | 1 Mbps |
| data bitrate | 5 Mbps |
| ping wire size (M→S on wire, external 側) | USB bulk — CAN 物理層を経由しない |
| echo wire size (S→M on wire, external 側) | USB bulk — 同上 |

external 側の wire time は USB bulk (1 ms frame) が支配。§9 の「bitrate 切替」
は peer 側の実験を対象にする。
...
- **provisioning が完了しない**: `fcan_hub_attach_self` が
...
- **echo latency が単一 bus 版より大きい**: 上記 "latency components" 節
  参照。設計上の下限は USB FS の 1 ms frame + hub poll。
