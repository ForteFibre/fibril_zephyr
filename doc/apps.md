...
その attach 自体を、アプリケーションは起動の最後に置く。
機能の `start` はすべてその前に走らせる。
attach はノードをバスに開く操作でもあるので、`start` を後に回すと、
ホストやピアからの要求が初期状態の publish と並んで走り、
命令したばかりの出力が上書きされうる。

`CONFIG_FCAN_TRANSPORT_GS_USB` は hub バックエンドに依存する。
直結のノードには被せる外部 port が無く、class を登録する者がいないまま
USB デバイスだけが build に入ることになるので、その組み合わせは選べない。
